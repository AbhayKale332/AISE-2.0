...
```
┌───────────────────────────── STAGE 1 — OPTICAL (Sentinel-2) ─────────────────────────────┐
│                                                                                            │
│  Sentinel-2 ZIPs ──► Scene discovery ──► Combined field/date extraction loop               │
│                                              │                                             │
│                              ┌───────────────┴────────────────┐                           │
│                              ▼                                ▼                           │
│                     ts (NDVI/NDWI/moisture)          field_obs_df (full spectral: B02–B12) │
│                              │                                │                            │
│                              ▼                                │                            │
│              Phenology feature extraction                     │                            │
│                              │                                │                            │
│                              ▼                                │                            │
│      PCA ──► KMeans (fixed k=5) ──► Hungarian cluster↔crop match                          │
│                              │                                │                            │
│                              ▼                                │                            │
│           weak_crop_type + weak_confidence  ──────────────────┤                            │
│                              │                                ▼                            │
│                              │                Merge crop_type BEFORE health scoring         │
│                              │                                │                            │
│                              │                                ▼                            │
│                              │            Crop-aware peer group construction                │
│                              │      (same crop+stage → same crop → same stage → flag)       │
│                              │                                │                            │
│                              │                                ▼                            │
│                              │       Peer-percentile health scoring (vigor/chlorophyll/     │
│                              │              moisture/trend weighted blend)                  │
│                              ▼                                ▼                             │
│                     final_field_assessment  (crop_type, health_score, health_status, …)    │
│                              │                                                              │
│                              ▼                                                              │
│         Export: CSV, GeoPackage, GeoTIFFs, maps, plots  +  SAR handoff files                │
│         (round1_farms.csv, Health_Score.csv, optical_crop_output, optical_health_output)    │
└────────────────────────────────────────────────┬─────────────────────────────────────────┘
                                                   │
                                                   ▼
┌───────────────────────────── STAGE 2 — SAR (Capella X-band) ─────────────────────────────┐
│                                                                                            │
│  Raw SAR SLC scenes ──► ESA SNAP `gpt` preprocessing graph (SLC → geocoded, calibrated,    │
│                          speckle-filtered Sigma0 dB GeoTIFFs)                              │
│                              │                                                             │
│                              ▼                                                             │
│                  Crop scenes to village boundary (per-scene GeoTIFFs)                      │
│                              │                                                             │
│                              ▼                                                             │
│    Per-field feature stack: backbone stats + GLCM texture + core/edge features             │
│    (build_farm_feature_stack) ── optionally joined to optical crop_type as context          │
│                              │                                                             │
│                              ▼                                                             │
│           Feature selection (top-N by |correlation| with health pseudo-label)              │
│                              │                                                             │
│                              ▼                                                             │
│   ┌─────────────────────────┴──────────────────────────┐                                  │
│   ▼                                                     ▼                                  │
│  SAR → crop-type classifier (XGBoost, multi:softprob)   SAR → health regressor (XGBoost)   │
│  target = optical weak_crop_type (no leakage)           target = optical health pseudo-lbl │
│   │                                                     │                                  │
│   ▼                                                     ▼                                  │
│  Cross-fitted crop probabilities / confidence / margin / entropy  ──► fed into 2nd health   │
│                                                                        model (SAR + soft-crop│
│                                                                        probabilities)        │
│                              │                                                             │
│                              ▼                                                             │
│         Nested CV, validation-selected blend of the two health models + yield estimate      │
│                              │                                                             │
│                              ▼                                                             │
│   final_sar_crop_health_yield (CSV/GPKG) + predicted_crop_type / health / confidence /      │
│   yield GeoTIFFs + combined 4-band stack + diagnostic plots + confusion matrix              │
└────────────────────────────────────────────────────────────────────────────────────────────┘
```
